...
export class AppComponent {
  ...
  books:Book[]=JSON.parse(JSON.stringify([{
    "ISBN": 9789000307975,
  ...
  noOfResults="Enter something to find books";
  onSearch(){
    this.results=[]
    let temp=this.query.value||'';
    for(let book of this.books){
      if(book.title.toLowerCase().includes(temp.toLowerCase())){
        this.results.push(book)
      }
    }
    console.log(this.query.value);
    if(this.query.value!="")
    this.noOfResults=this.results.length.toString()+" Results Found";
    else{ this.noOfResults="Enter something to find books";this.results=[]}
}
}
